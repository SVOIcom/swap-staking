...
class Button extends _UIComponent {
    ...
    async buildHtml() {
        ...
        return `
            <button id="${this.id}" style="${this._visible?'':'display: none;'} ${this.attrs.style??this.attrs.style}" class="btn  ${this.attributes.flat ? 'btn-flat' : 'btn-default'} ${this.attributes.right ? 'float-right' : ''} ${this.attributes.large ? 'btn-lg' : ''} ${this._disabled ? 'disabled' : ''} ${this.attributes.block ? 'btn-block' : ''}  "   ${this.attributes.href ? 'href="' + this.attributes.href.value + '"' : ''} ${attribStr}>
                ${this._caption}
            </button>`;
    }
